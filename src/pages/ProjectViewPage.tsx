  ...
  return (
    <div className="min-h-screen bg-[#f5f2eb] text-slate-800 pb-12">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        {loading ? (
          <div className="py-24 flex flex-col justify-center items-center space-y-3 bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="glass-panel rounded-2xl p-8 text-center bg-white">
            ...
            <h3 className="font-bold text-slate-900 text-lg mb-2">Access Denied / Error</h3>
            <p className="text-sm text-slate-500 mb-6 max-w-md mx-auto">{error}</p>
            <Link
              to="/dashboard"
              className="inline-flex justify-center px-4 py-2 border border-slate-200 rounded-xl bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 transition"
            >
              Return to Dashboard
            </Link>
          </div>
          ...
          <div className="glass-panel rounded-2xl p-8 bg-white">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-6 mb-6 gap-4">
              <div>
                <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest block mb-1">Project Workspace</span>
                <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 leading-tight">{project.title}</h1>
              </div>
              <div className="flex items-center space-x-2.5">
            ...
            {!canEdit && (
              <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 text-sm text-amber-700 flex items-start space-x-3 mb-6">
            ...
            {saveSuccess && (
              <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-sm text-emerald-700 flex items-center space-x-3 mb-6 animate-pulse">
            ...
            {saveError && (
              <div className="rounded-xl bg-rose-50 border border-rose-200 p-4 text-sm text-rose-600 flex items-start space-x-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{saveError}</p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="block w-full px-3 py-3 border border-slate-200 rounded-xl bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition duration-150 ease-in-out text-sm disabled:opacity-60 disabled:cursor-not-allowed"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select
                    ...
                    className="block w-full px-3 py-3 border border-slate-200 rounded-xl bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition duration-150 ease-in-out text-sm disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-3 py-3 border border-slate-200 rounded-xl bg-slate-50 text-slate-500 text-sm font-mono cursor-not-allowed opacity-60"
                  />
                </div>
              </div>
                ...
                <div className="flex items-center space-x-3 self-end sm:self-auto">
                  ...
                  {user?.role === 'admin' && (
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-600 hover:text-white hover:border-transparent transition text-xs font-semibold cursor-pointer"
                    >
                      Delete Project
                    </button>
                  )}
      ...
      {showDeleteModal && project && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md rounded-2xl overflow-hidden p-6 relative animate-in fade-in zoom-in-95 duration-150 bg-white">
            <h3 className="text-lg font-bold text-slate-900 mb-2">Delete Project</h3>
            <p className="text-sm text-slate-600 mb-6">
              Are you sure you want to delete <span className="font-semibold text-slate-900">"{project.title}"</span>? This action is permanent and cannot be undone.
            </p>
